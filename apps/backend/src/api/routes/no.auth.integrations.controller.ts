...
export class NoAuthIntegrationsController {
  ...
  async connectSocialMedia(
    @Param('integration') integrationParam: string,
    @Body() body: ConnectIntegrationDto
  ) {
    let integration = integrationParam;
    if (body.state) {
      const intended = await ioRedis.get(`oauth_integration:${body.state}`);
      if (
        intended &&
      ...
      await ioRedis.del(`oauth_integration:${body.state}`);
    }
