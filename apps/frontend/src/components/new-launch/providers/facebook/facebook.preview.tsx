...
import { usePostAnalytics } from '@gitroom/frontend/components/platform-analytics/use.post.analytics';
...
export const FacebookPreview: FC<{
  ...
  const { totals } = usePostAnalytics(topValue?.[0]?.id);
    ...
    <div className="py-[15px] flex flex-col px-[15px] w-full gap-[20px] bg-bgFacebook rounded-[12px]">
      ...
      <div className="flex text-textLinkedin text-[12px] font-[400] items-center">
        <div className="flex flex-1 gap-[10px] items-center">
          <Icons />
          <div className="">{totals.likes.toLocaleString()}</div>
        </div>
        <div className="gap-[9px] items-center flex">
          <div>{totals.comments.toLocaleString()} Comments</div>
        </div>
      </div>
