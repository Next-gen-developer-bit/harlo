...
export const COMPOSER_LOCAL_DRAFT_KEY = 'poscally-composer-unsaved';
...
export const ManageModal: FC<AddEditModalProps> = (props) => {
  ...
  const deletePost = useCallback(async () => {
    ...
    modal.closeAll();
    return;
  }, [existingData, fetch, modal, mutate, t, toaster]);

  const schedule = useCallback(
    (
        type: 'draft' | 'now' | 'schedule' | 'update',
        dateOverride?: dayjs.Dayjs
      ) =>
      async () => {
        ...
        const existingState = existingData?.posts?.[0]?.state;
        if (type === 'draft' && existingState === 'PUBLISHED') {
          toaster.show(
            t(
              'published_posts_cannot_become_drafts',
        ...
        if (
          type === 'draft' &&
          (existingState === 'QUEUE' || existingState === 'ERROR')
        ) {
        ...
        const group = groupRef.current || existingData.group || makeId(10);
        ...
        if (!dummy) {
          if (addEditSets) {
            ...
            toaster.show(
              type === 'draft' &&
                (existingState === 'QUEUE' || existingState === 'ERROR')
                ? t('moved_to_drafts', 'Moved to drafts')
  ...
  const addToQueue = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !slot?.date) {
        throw new Error(
          slot?.message ||
            t('could_not_find_queue_slot', 'Could not find a queue slot')
        );
      }

      const queueDate = dayjs.utc(slot.date).local();
      setDate(queueDate);
      setLoading(false);
      await schedule('schedule', queueDate)();
    } catch (error) {
      setLoading(false);
      toaster.show(
  ...
  }, [fetch, schedule, setDate, t, toaster]);

  const saveDraftSilent = useCallback(async () => {
    if (dummy || locked || loading || autoSaving.current || addEditSets) {
      return;
    }
    const existingState = existingData?.posts?.[0]?.state;
    ...
    const persistLocal = (values: any[]) => {
      try {
        sessionStorage.setItem(
          COMPOSER_LOCAL_DRAFT_KEY,
          JSON.stringify({
            global: values,
            tags,
    ...
    if (!selectedIntegrations.length || !ref.current?.getAllValues) {
      if ((global || []).some((value) =>
    ...
    const saveRequest = (async () => {
      const allValues = await ref.current.getAllValues();
      const hasContent = (allValues || []).some((post: any) =>
        (post.values || []).some(
          (value: any) =>
            (value.content || '').replace(/<[^>]*>/g, '').trim() ||
            (value.media || []).length
      ...
      if (res.ok) {
        const savedPosts = await res.json().catch(() => []);
        if (savedPosts?.[0]?.group) {
          groupRef.current = savedPosts[0].group;
        }
        setDraftSaved(true);
        try {
          sessionStorage.removeItem(COMPOSER_LOCAL_DRAFT_KEY);
        } catch {
          // Ignore storage access errors.
        }
      }
    })();
    autoSavePromise.current = saveRequest;

    try {
      await saveRequest;
    } catch {
      // Keep typing responsive; the next debounce retries.
    } finally {
