...
const isVideoMedia = (path?: string | null) =>
  ['mp4', 'mov', 'webm'].some((extension) =>
    hasMediaExtension(path, extension)
  );
export const MediaBox: FC<{
  ...
  return (
    <DropFiles
      disabled={loading}
      className={clsx('flex flex-col flex-1', standalone && 'min-h-[520px]')}
      onDrop={dragAndDrop}
    >
      <div className="flex flex-col flex-1">
        ...
        >
          <div
            className={clsx(
              standalone
                ? 'min-h-[400px] overflow-y-auto p-1'
              ...
              standalone &&
                ...
                'grid auto-rows-max grid-cols-1 content-start gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4',
              !isLoading &&
                !data?.results?.length &&
              ...
              <>
                {[...new Array(16)].map((_, i) => (
                  <div
                    className={clsx(
                      standalone
                        ? 'aspect-square rounded-xl bg-newSep animate-pulse'
            ...
            {data?.results
              ...
              .map((media: any) =>
                standalone ? (
                  <div
                    key={media.id}
                    className="group min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md"
                  >
                    <div className="relative aspect-square overflow-hidden bg-slate-100">
                      <button
                        ...
                        className="absolute right-2 top-2 z-30 rounded-full bg-white p-1 shadow-md"
                      >
                        <DeleteCircleIcon className="h-6 w-6 cursor-pointer" />
                      </button>
                      {isVideoMedia(media.path) ? (
                        <VideoFrame url={mediaDirectory.set(media.path)} />
                      ) : (
                        <>
                          <div className="absolute inset-0 flex items-center justify-center px-4 text-center text-xs font-medium text-slate-500">
                            Preview unavailable
                          </div>
                          <img
                            className="relative z-10 h-full w-full bg-slate-100 object-cover"
                            src={mediaDirectory.set(media.path)}
...
export const MediaComponent: FC<{
  ...
  return (
    <div className="flex flex-col gap-[8px]">
      ...
      <div className="text-[12px] text-newTableText">{description}</div>
      {!!currentMedia && (
        <div className="mt-[8px] cursor-pointer w-[160px] h-[160px] rounded-[8px] overflow-hidden border border-newTableBorder">
          <img
            className="w-full h-full object-cover"
      ...
      <div className="flex flex-wrap gap-[10px] mt-[4px]">
        <Button className="rounded-[8px]" onClick={showModal}>
          {currentMedia
