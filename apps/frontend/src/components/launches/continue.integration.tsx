...
export const ContinueIntegration: FC<{
  ...
  const [connectProvider, setConnectProvider] = useState(provider);
  ...
  const modifiedParams = useMemo(() => {
    ...
    if (provider === 'x' && searchParams.oauth_verifier) {
      return {
    ...
    (async () => {
      if (searchParams.error) {
        const description = searchParams.error_description
          ? decodeURIComponent(
              String(searchParams.error_description).replace(/\+/g, ' ')
            )
      ...
      if (providerIdentifier) {
        setConnectProvider(providerIdentifier);
      }
  ...
  const Provider = useMemo(() => {
    return (
      continueProviderList[
        connectProvider as keyof typeof continueProviderList
      ] || null
    );
  }, [connectProvider]);
  ...
  const providerDisplayName = useMemo(() => {
    ...
    return names[connectProvider] || connectProvider;
  }, [connectProvider]);
