...
export const Menu: FC<{
  ...
  // Keep the menu inside the viewport after it measures itself
  useLayoutEffect(() => {
    ...
    if (
      changed &&
      (Math.abs(show.x - next.x) > 1 || Math.abs(show.y - next.y) > 1)
    ) {
      setShow(next);
    }
  }, [show]);
  ...
  const changeShow: MouseEventHandler<HTMLDivElement> = useCallback(
    (e) => {
      ...
      let x = trigger.right - estimatedWidth;
      if (x < padding) {
        x = padding;
      }
    ...
    >
      ...
      {show &&
        ...
        <div
          ...
          className="fixed z-[300] min-w-[280px] w-max max-w-[calc(100vw-24px)] p-2 bg-white shadow-lg flex flex-col gap-0.5 rounded-xl border border-slate-200"
        >
          {canDisable && !findIntegration?.refreshNeeded && (
            <div
              className="flex gap-[12px] items-center py-[8px] px-[10px] rounded-lg hover:bg-slate-50 cursor-pointer"
            ...
            >
              ...
              <div className="text-[14px] font-medium text-slate-800 whitespace-nowrap">
          ...
          >
            ...
            <div className="text-[14px] font-medium text-slate-800 whitespace-nowrap">{t('copy_id', 'Copy Channel ID')}</div>
          </div>
          {canDisable &&
          ...
          >
            ...
            <div className="text-[14px] font-medium text-red-500 whitespace-nowrap">
              {t('delete', 'Delete')}
            </div>
          </div>
        </div>,
