...
export const CalendarWeekProvider: FC<{
  ...
  const calendarDisplay =
    urlDisplay === 'day' || urlDisplay === 'week' || urlDisplay === 'month'
      ? urlDisplay
      : displaySaved === 'day' || displaySaved === 'week'
      ? displaySaved
      : 'month';
  const display = stateParam ? 'list' : calendarDisplay;
  ...
  const setFiltersWrapper = useCallback(
    (newFilters: {
      ...
      if (newFilters.display === 'list') {
        setListPage(0);
        router.replace('/launches?state=all');
        return;
      }
      ...
      router.replace(`/launches?${path.join('&')}`);
    },
    [router]
  );
  ...
  useEffect(() => {
    ...
    const range =
      ...
      urlDisplay !== 'list'
        ...
        : getDateRange(calendarDisplay);

    setFilters((current) => {
      if (
        current.display === calendarDisplay &&
  ...
  }, [searchKey, stateParam, calendarDisplay, urlDisplay]);
