...
export class UsersService {
      ...
      await Promise.all(
        [kept, switched].map((account) =>
          this._notificationService
            .sendEmail(
              account.email,
              'Your Poscally login was changed',
              `An administrator changed the login for your Poscally account. ` +
  ...
  async submitFeedback(
    ...
    const subject = `Poscally feedback${category ? ` (${category})` : ''}`;
    ...
    const supportInbox = process.env.EMAIL_FROM_ADDRESS;
    if (supportInbox && this._notificationService.hasEmailProvider()) {
      await this._notificationService.sendEmail(
        supportInbox,
        subject,
        `<p><strong>From:</strong> ${email}</p><p><strong>Category:</strong> ${
          category || 'general'
        }</p><p>${message.replace(/\n/g, '<br />')}</p>`
      );
    }
