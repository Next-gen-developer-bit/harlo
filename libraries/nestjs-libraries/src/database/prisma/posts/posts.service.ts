...
const ANALYTICS_SNAPSHOT_KEY = '__analyticsSnapshot';
...
export class PostsService {
  ...
  async checkPostAnalytics(
    ...
    try {
      const loadAnalytics = await Promise.race([
        ...
        new Promise<AnalyticsData[]>((_, reject) =>
          setTimeout(() => reject(new Error('analytics timeout')), 12000)
        ),
      ]);
      if (Array.isArray(loadAnalytics) && loadAnalytics.length > 0) {
        await ioRedis.set(
          `integration:${orgId}:${post.id}:${date}`,
  ...
  private summarizeAnalytics(analytics: AnalyticsData[]) {
    const likes = this.pickAnalyticsMetric(analytics, [
      'Likes',
      'Reactions',
      'Pin Clicks',
    ]);
    const comments = this.pickAnalyticsMetric(analytics, [
      'Comments',
      'Replies',
    ]);
    const shares = this.pickAnalyticsMetric(analytics, [
      'Shares',
      'Reposts',
      'Retweets',
      'Saves',
    ]);
    const impressions = this.pickAnalyticsMetric(analytics, [
      'Impressions',
      'Views',
      'Reach',
      'Unique Impressions',
    ]);
    const engagementParts = [likes, comments, shares].filter(
    ...
    return {
      ...
      engagements:
        engagementParts.length > 0
          ? engagementParts.reduce((acc, value) => acc + value, 0)
          : null,
    };
  }

  private parseAnalyticsSnapshot(settings?: string | null) {
    try {
      const parsed = JSON.parse(settings || '{}');
      const snapshot = parsed?.[ANALYTICS_SNAPSHOT_KEY];
  ...
  private mergeAnalyticsMetrics(
    ...
    const impressions = live.impressions ?? snapshot?.impressions ?? null;
    const likes = live.likes ?? snapshot?.likes ?? null;
    const comments = live.comments ?? snapshot?.comments ?? null;
    const shares = live.shares ?? snapshot?.shares ?? null;
    const engagementParts = [likes, comments, shares].filter(
      (value): value is number => value !== null
    );
  ...
  async getPublishedPostsAnalytics(orgId: string, date: number) {
    const days = [7, 30, 90].includes(date) ? date : 30;
    const posts = await this._postRepository.getPublishedPostsForAnalytics(
      orgId,
      dayjs.utc().subtract(days, 'day').toDate()
    );
      ...
      mapped.push(
        ...(await Promise.all(
          chunk.map(async (post) => {
            const provider = post.integration
              ? this._integrationManager.getSocialIntegration(
                  post.integration.providerIdentifier
                )
              : undefined;
            const analytics = await this.checkPostAnalytics(
              orgId,
              post.id,
              days
            );
            const live = Array.isArray(analytics)
            ...
            const snapshot = this.parseAnalyticsSnapshot(
              (post as { settings?: string | null }).settings
            );
            const metrics = this.mergeAnalyticsMetrics(live, snapshot);
            const available =
              metrics.impressions !== null ||
              metrics.likes !== null ||
              metrics.comments !== null ||
              metrics.shares !== null;
            const canImportStats = Boolean(provider?.analyticsManualImport);
            ...
            return {
              ...
              ...metrics,
              available,
              reachLabel: provider?.analyticsReachLabel || 'Impressions',
              unavailableHint:
                available || !provider?.analyticsUnavailableHint
                  ? ''
                  : provider.analyticsUnavailableHint,
              canImportStats,
              importHint: canImportStats
                ? provider?.analyticsImportHint || ''
                : '',
              importUrl: canImportStats
                ? provider?.analyticsImportUrl?.(
                    post.releaseURL,
                    post.releaseId
                  ) ||
                  post.releaseURL ||
    ...
    const totals = {
      impressions: sumAvailable(mapped.map((post) => post.impressions)),
      engagements: sumAvailable(mapped.map((post) => post.engagements)),
      likes: sumAvailable(mapped.map((post) => post.likes)),
      comments: sumAvailable(mapped.map((post) => post.comments)),
      shares: sumAvailable(mapped.map((post) => post.shares)),
        ...
        (all, post) => {
          ...
          if (!all[id]) {
            all[id] = {
              ...
              reachLabel: post.reachLabel,
            };
          }
          all[id].posts += 1;
          if (post.impressions !== null) {
            all[id].impressions = (all[id].impressions || 0) + post.impressions;
          }
          if (post.engagements !== null) {
            all[id].engagements = (all[id].engagements || 0) + post.engagements;
          }
          return all;
        },
        {} as Record<
    ...
    return { posts: mapped, totals, channels };
  }

  async saveAnalyticsSnapshot(
    orgId: string,
    postId: string,
    body: AnalyticsSnapshotDto
  ) {
    const values = {
    ...
    if (
      ...
      throw new BadRequestException('Add at least one metric');
    }

    const post = await this._postRepository.getPost(postId, true, orgId, true);
    if (!post || post.parentPostId || !post.integration) {
      throw new NotFoundException('Post not found');
    }
    if (post.state !== 'PUBLISHED') {
      throw new BadRequestException('Only published posts can store stats');
    }

    const provider = this._integrationManager.getSocialIntegration(
      post.integration.providerIdentifier
    );
    if (!provider?.analyticsManualImport) {
      throw new BadRequestException(
        'This channel does not support imported stats'
      );
    }

    let existingSettings: Record<string, any> = {};
    try {
      existingSettings = JSON.parse(post.settings || '{}');
    } catch {
      existingSettings = {};
    }

    const existingSnapshot =
      existingSettings[ANALYTICS_SNAPSHOT_KEY] &&
      typeof existingSettings[ANALYTICS_SNAPSHOT_KEY] === 'object'
        ? existingSettings[ANALYTICS_SNAPSHOT_KEY]
        : {};
    ...
    const snapshot = {
      ...
      ...(values.impressions !== undefined
        ...
        : {}),
      ...(values.likes !== undefined ? { likes: values.likes } : {}),
      ...(values.comments !== undefined ? { comments: values.comments } : {}),
      ...(values.shares !== undefined ? { shares: values.shares } : {}),
      updatedAt: new Date().toISOString(),
    };

    await this._postRepository.updatePostSettings(
      post.id,
      orgId,
      JSON.stringify({
        ...existingSettings,
        [ANALYTICS_SNAPSHOT_KEY]: snapshot,
      })
    );

    await Promise.all(
      [7, 30, 90].map((days) =>
        ioRedis.del(`integration:${orgId}:${post.id}:${days}`)
      )
    );
  ...
  getWorkspaceOverview(orgId: string) {
    return this._postRepository.getWorkspaceOverview(orgId);
  }
  ...
  async deletePost(orgId: string, group: string) {
    ...
    if (post?.id) {
      void this.terminatePostWorkflows(post.id);
    }
  ...
  private getTemporalWorkflowClient() {
    const client = this._temporalService.client?.getRawClient();
    if (!client?.workflow) {
      throw new BadRequestException(
        'Scheduling is not available because the job runner is not connected. Start Temporal and the orchestrator, then try again.'
      );
    }
    return client;
  }
  ...
  async startWorkflow(
    ...
    const client = this.getTemporalWorkflowClient();
    ...
    try {
      ...
      console.error(`Failed to start post workflow for ${postId}`, err);
      Sentry.captureException(err);
      throw new BadRequestException(
        'Could not schedule this post because the job runner is not connected. Start Temporal and the orchestrator, then try again.'
      );
    }
  }
  ...
  async createPost(
    ...
    const sourceGroups = [
      ...new Set(body.posts.map((post) => post.group).filter(Boolean)),
    ] as string[];
    const nextGroup = keepGroup ? undefined : makeId(10);
    if (body.type === 'schedule' || body.type === 'now') {
      this.getTemporalWorkflowClient();
    }
    for (const post of body.posts) {
      ...
      retainedPostIds.push(...posts.map((createdPost) => createdPost.id));
    ...
    for (const group of sourceGroups) {
      await this._postRepository.cleanupPostGroup(
        group,
        keepGroup,
        retainedPostIds
      );
  ...
  async changePostStatus(
    ...
    for (const post of groupPosts) {
      ...
      await this._postRepository.changeState(post.id, state);
    }
  ...
  async retryFailedPost(orgId: string, id: string) {
    const getPostById = await this._postRepository.getPostById(id, orgId);
    if (!getPostById) {
      throw new NotFoundException('Post not found');
    }

    if (getPostById.state !== 'ERROR') {
      throw new BadRequestException('Only failed destinations can be retried');
    }

    if (!getPostById.integration) {
      throw new BadRequestException(
        'This destination no longer has a connected account'
      );
    }

    if (getPostById.integration.refreshNeeded) {
      throw new BadRequestException(
        'Reconnect this account, then retry this destination'
      );
    }

    await this._postRepository.retryFailedPost(orgId, id);
    ...
    return { id, state: 'QUEUE' as State };
  }
