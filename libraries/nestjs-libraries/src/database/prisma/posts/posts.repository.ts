...
export class PostsRepository {
  ...
  async getWorkspaceOverview(orgId: string) {
    const baseWhere = {
      organizationId: orgId,
      deletedAt: null as Date | null,
      parentPostId: null as string | null,
    ...
    const last7Start = dayjs().subtract(6, 'day').startOf('day').toDate();

    const [counts, upcoming, failedPosts, weekPosts, recentPosted] =
      await Promise.all([
        ...
        this._post.model.post.findMany({
          where: {
            ...baseWhere,
            state: { in: [State.QUEUE, State.PUBLISHED, State.ERROR] },
            publishDate: { gte: weekStart, lte: weekEnd },
          },
          orderBy: { publishDate: 'asc' },
          take: 80,
          select: upcomingSelect,
    ...
    const dailyPosted = [...Array(7)].map((_, index) => {
      const day = dayjs().subtract(6 - index, 'day');
      return {
        date: day.format('YYYY-MM-DD'),
        count: recentPosted.filter((post) =>
          dayjs(post.publishDate).isSame(day, 'day')
        ).length,
      };
    });

    return {
      drafts: countMap[State.DRAFT] || 0,
  ...
  updatePostSettings(id: string, orgId: string, settings: string) {
    return this._post.model.post.update({
      where: {
        id,
        organizationId: orgId,
  ...
  async changeState(id: string, state: State, err?: any, body?: any) {
    const update = await this._post.model.post.update({
      ...
      data: {
        state,
        ...(err ? { error: readablePostError(err) } : {}),
      },
      include: {
        integration: {
          select: {
  ...
  async createOrUpdatePost(
    ...
    const posts: Post[] = [];
    const uuid = uuidv4();
    const group = keepGroup && body.group ? body.group : nextGroup || uuid;

    for (const value of body.value) {
      const existingPostId = value.id
        ? (
        ...
        : undefined;
      const postId = existingPostId || uuidv4();
  ...
  async cleanupPostGroup(
    group: string,
    keepGroup: boolean,
    retainedPostIds: string[]
  ) {
    await this._post.model.post.updateMany({
      where: {
        group,
  ...
  getPublishedPostsForAnalytics(orgId: string, since: Date) {
    return this._post.model.post.findMany({
      where: {
        organizationId: orgId,
        deletedAt: null,
