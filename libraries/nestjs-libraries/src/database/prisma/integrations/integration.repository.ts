...
export class IntegrationRepository {
  ...
  async updateIntegration(id: string, params: Partial<Integration>) {
    ...
    const mergeIntoExisting = Boolean(existing && existing.id !== id);
    ...
    return this._integration.model.integration.update({
      where: {
        ...(mergeIntoExisting ? { id: existing!.id } : { id }),
      },
      data: {
        ...params,
        disabled: false,
  ...
  async createOrUpdateIntegration(
    ...
    let upsertInternalId = internalId;
    if (isBetweenSteps && !refresh) {
      const occupied = await this._integration.model.integration.findFirst({
        where: {
          organizationId: org,
          internalId,
          deletedAt: null,
      ...
      if (occupied) {
        upsertInternalId = `${provider}:pending:${internalId}`;
      }
    }
  ...
  findUnpublishedPostIdsForChannel(org: string, id: string) {
