...
// Use with `ssrfSafeFetch`, not global fetch: Node 26's built-in fetch drops
// all response headers when given an Agent from the npm `undici` package.
...
// Self-hosters legitimately connect Poscally to WordPress/Mastodon/Lemmy/Listmonk
...
export function ssrfSafeFetch(
  ...
  const dispatcher = (init as any).dispatcher ?? getSsrfSafeDispatcher();
  if (!dispatcher) {
