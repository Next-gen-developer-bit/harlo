...
export class YoutubeProvider extends SocialAbstract implements SocialProvider {
  ...
  override analyticsReachLabel = 'Views';
  identifier = 'youtube';
  name = 'YouTube';
  isBetweenSteps = true;
  ...
  // Delegates to SocialAbstract.mediaSize so HEAD-without-Content-Length
  // stores (Supabase/S3) use the same Range probe as other providers.
  ...
  async postAnalytics(
    ...
    const videoId = this.youtubeVideoId(postId);
    if (!videoId) {
      return [];
    }
  ...
  private youtubeVideoId(postId: string): string {
