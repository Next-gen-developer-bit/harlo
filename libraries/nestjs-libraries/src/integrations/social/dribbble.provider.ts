...
export class DribbbleProvider extends SocialAbstract implements SocialProvider {
  ...
  async refreshToken(refreshToken: string): Promise<AuthTokenDetails> {
    // Dribbble issues long-lived access tokens and has no refresh-token flow
    // (see authenticate() below, which stores expiresIn: 999999999). There is
    // nothing to refresh - report an empty result so a dead token prompts a
    // reconnect instead of calling another provider's API.
    return {
      refreshToken: '',
      expiresIn: 0,
      accessToken: '',
