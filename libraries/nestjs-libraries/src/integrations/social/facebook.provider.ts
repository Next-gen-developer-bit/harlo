...
export class FacebookProvider extends SocialAbstract implements SocialProvider {
    ...
    return {
      url:
        ...
        `&redirect_uri=${encodeURIComponent(
          this.oauthRedirect('/integrations/social/facebook')
        )}` +
        `&state=${state}` +
  ...
  async postAnalytics(
    ...
    try {
      ...
      const pushMetric = (label: string, total: number | string) => {
      ...
      for (const metric of data || []) {
        ...
        switch (metric.name) {
          case 'post_total_media_view_unique':
            pushMetric('Impressions', value);
            break;
          case 'post_clicks':
            pushMetric('Clicks', value);
            break;
          case 'post_clicks_by_type':
            if (typeof value === 'object') {
              ...
              pushMetric('Clicks by Type', totalClicks);
            }
            break;
          case 'post_reactions_by_type_total':
            if (typeof value === 'object') {
              ...
              pushMetric('Reactions', totalReactions);
            }
            break;
        }
      }

      const objectStats = await (
        await fetch(
          `https://graph.facebook.com/v23.0/${postId}?fields=shares,comments.summary(true),reactions.summary(true)&access_token=${accessToken}`
        )
      ).json();

      if (objectStats?.reactions?.summary?.total_count !== undefined) {
        pushMetric('Reactions', objectStats.reactions.summary.total_count);
      }
      if (objectStats?.comments?.summary?.total_count !== undefined) {
        pushMetric('Comments', objectStats.comments.summary.total_count);
      }
      if (objectStats?.shares?.count !== undefined) {
        pushMetric('Shares', objectStats.shares.count);
      }
