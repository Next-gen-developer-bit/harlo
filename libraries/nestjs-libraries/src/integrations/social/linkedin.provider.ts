...
export class LinkedinProvider extends SocialAbstract implements SocialProvider {
  ...
  override analyticsUnavailableHint =
    'LinkedIn personal profiles do not expose impressions through the posting API. Add the numbers from LinkedIn analytics, or connect a LinkedIn Page for automatic stats.';
  override analyticsManualImport = true;
  override analyticsImportHint =
    'Log in as the profile that published the post. Open LinkedIn analytics, copy Impressions, Reactions, Comments, and Reposts, then save them here.';

  override analyticsImportUrl(
    releaseURL?: string | null,
    releaseId?: string | null
  ): string {
    if (releaseId) {
      return `https://www.linkedin.com/analytics/post-summary/${encodeURIComponent(
        releaseId
      )}`;
    }
    return (
      releaseURL || 'https://www.linkedin.com/analytics/creator/content'
    );
  }
  ...
  protected linkedinRedirectUri(refresh?: string) {
    const origin = (process.env.FRONTEND_URL || '').replace(/\/$/, '');
    return `${origin}/integrations/social/${this.identifier}${
  ...
  async authenticate(params: {
    ...
    body.append(
      'redirect_uri',
      this.linkedinRedirectUri(params.refresh)
    );
    ...
    this.checkScopes(this.requestedScopes(), scope);
  ...
  private async processMediaForPosts(
    ...
    for (const post of postDetails) {
      for (const media of post.media || []) {
        ...
        // Personal image/document URNs cannot be polled; a short pause after
        // the upload is enough for LinkedIn to accept the asset. A durable
        // graceChecks cycle would add a 20s workflow sleep after the scheduled
        // time, which made posts land late.
        if (uploaded.grace) {
          await timer(2000);
        }
      }
  ...
  private async createMainPost(
    ...
    const postId = this.readCreatedPostId(response);
    if (!postId) {
      let body = '{}';
      try {
      ...
      throw new BadBody(
        ...
        body,
        JSON.stringify(postPayload),
        'LinkedIn accepted the request but did not return a post ID'
      );
    }
  ...
  private readCreatedPostId(response: Response): string | undefined {
    const headerId =
    ...
    const urn = decoded.match(/urn:li:(?:share|ugcPost):[^/?#\s]+/);
    if (urn?.[0]) {
      return urn[0];
    }

    const segment = decoded.split('/').filter(Boolean).pop();
    if (segment && segment !== 'posts' && segment !== 'null') {
      return segment;
    }
  ...
  override async finalizePost(
    ...
    // Mutation activities are not retried (maximumAttempts: 1). Arming here
    // and waiting for the next 20s check cycle only delayed LinkedIn posts
    // after the scheduled time. A timeout after this create is treated as
    // unconfirmed, same as a timed-out handshake create.
    if (pendingData.attempting && pendingData.confirmed) {
      throw new BadBody(
        this.identifier,
        '{}',
  ...
  async postAnalytics(
    ...
    const memberAnalytics = await this.memberCreatorPostAnalytics(
      accessToken,
      postId
    );
    ...
    return this.linkedinEngagementAnalytics(accessToken, postId);
  }

  private requestedScopes() {
    if (process.env.LINKEDIN_MEMBER_POST_ANALYTICS === 'true') {
      return [...this.scopes, 'r_member_postAnalytics'];
    }
    return this.scopes;
  }

  private linkedinRestHeaders(accessToken: string) {
    return {
      Authorization: `Bearer ${accessToken}`,
      'LinkedIn-Version': '202601',
      'X-Restli-Protocol-Version': '2.0.0',
  ...
  private isLinkedInErrorBody(body: any) {
    ...
    return Boolean(body.serviceErrorCode || body.code === 'ACCESS_DENIED');
  }

  private async linkedinGetJson(url: string, accessToken: string) {
    try {
      const response = await ssrfSafeFetch(url, {
        method: 'GET',
        headers: this.linkedinRestHeaders(accessToken),
      });
      const body = await response.json();
      if (!response.ok || this.isLinkedInErrorBody(body)) {
        return null;
      }
      return body;
    } catch {
  ...
  private memberPostAnalyticsEntity(postId: string) {
    if (String(postId).includes(':share:')) {
      return `(share:${encodeURIComponent(postId)})`;
    }
    return `(ugc:${encodeURIComponent(postId)})`;
  }

  private linkedinMetricType(metricType: unknown) {
    if (typeof metricType === 'string') {
      return metricType;
    }
    if (metricType && typeof metricType === 'object') {
      const value = Object.values(metricType as Record<string, unknown>).find(
  ...
  private async memberCreatorMetric(
    accessToken: string,
    postId: string,
    queryType: string
  ) {
    const body = await this.linkedinGetJson(
      `https://api.linkedin.com/rest/memberCreatorPostAnalytics?q=entity&entity=${this.memberPostAnalyticsEntity(
        postId
    ...
    return elements.reduce(
      (sum: number, element: { count?: number; metricType?: unknown }) => {
        const metricType = this.linkedinMetricType(element?.metricType);
        if (metricType && metricType !== queryType) {
          return sum;
        }
        return sum + Number(element?.count || 0);
      },
      0
    );
  }

  private async memberCreatorPostAnalytics(
    accessToken: string,
    postId: string
  ): Promise<AnalyticsData[]> {
    ...
    const rows = await Promise.all(
      metrics.map(async ([queryType, label]) => {
        const total = await this.memberCreatorMetric(
          accessToken,
          postId,
          queryType
        );
  ...
  private mapSocialMetadataToAnalytics(metadata: any, today: string) {
    const likes = Object.values(
      (metadata?.reactionSummaries || {}) as Record<string, { count?: number }>
    ).reduce((sum, item) => sum + Number(item?.count || 0), 0);
    const comments =
      metadata?.commentSummary?.count ||
      metadata?.commentSummary?.topLevelCount ||
      0;
    ...
    return [
      {
        label: 'Likes',
        percentageChange: 0,
        data: [{ total: String(likes), date: today }],
      },
      {
        label: 'Comments',
        percentageChange: 0,
        data: [{ total: String(comments), date: today }],
      },
    ];
  }

  private async linkedinEngagementAnalytics(
    accessToken: string,
    postId: string
  ): Promise<AnalyticsData[]> {
    const today = dayjs().format('YYYY-MM-DD');
    ...
    const socialActions = await this.linkedinGetJson(
      `https://api.linkedin.com/v2/socialActions/${encodedPostId}`,
      accessToken
    );
    if (socialActions?.likesSummary || socialActions?.commentsSummary) {
      return this.mapSocialActionsToAnalytics(socialActions, today);
    }

    const metadata =
      (await this.linkedinGetJson(
        `https://api.linkedin.com/rest/socialMetadata/${encodedPostId}`,
        accessToken
      )) ||
      (await this.linkedinGetJson(
        `https://api.linkedin.com/rest/socialMetadata?ids=List(${encodedPostId})`,
        accessToken
      ));

    const metadataForPost =
      metadata?.results?.[postId] ||
      metadata?.results?.[encodedPostId] ||
      metadata?.[postId] ||
      metadata;

    if (
      metadataForPost?.reactionSummaries ||
      metadataForPost?.commentSummary
    ) {
      return this.mapSocialMetadataToAnalytics(metadataForPost, today);
    }

    return [];
  }

  private mapSocialActionsToAnalytics(socialActions: any, today: string) {
    const likes = socialActions?.likesSummary?.totalLikes || 0;
    const comments =
      socialActions?.commentsSummary?.aggregatedTotalComments ||
      socialActions?.commentsSummary?.totalFirstLevelComments ||
      0;
    const shares =
      socialActions?.sharesSummary?.totalShares ||
      socialActions?.reshareStatistics?.shareCount ||
      0;
    ...
    return [
      ...
      },
      {
        label: 'Shares',
        percentageChange: 0,
        data: [{ total: String(shares), date: today }],
      },
    ];
  }
