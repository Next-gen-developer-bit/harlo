  ...
  implements SocialProvider
{
  ...
  override analyticsManualImport = false;
  ...
  private organizationIdFromUrn(urn?: string) {
    if (!urn) {
      return '';
    }
  ...
  async companies(accessToken: string) {
    ...
    const aclBody = await aclResponse.json().catch(() => ({}));
    const elements = Array.isArray(aclBody?.elements) ? aclBody.elements : [];

    if (!aclResponse.ok) {
      console.error('LinkedIn organizationAcls failed', {
    ...
    const pages = await Promise.all(
      elements
        .filter((entry: any) => allowedRoles.has(entry.role))
        .map(async (entry: any) => {
          const orgId = this.organizationIdFromUrn(
            entry.organization || entry.organizationalTarget
          );
          if (!orgId) {
            return null;
          }
          ...
          const decorated =
            entry['organization~'] || entry['organizationalTarget~'] || {};
          let name = decorated.localizedName;
          let username = decorated.vanityName;
          let picture =
            decorated.logoV2?.['original~']?.elements?.[0]?.identifiers?.[0]
              ?.identifier;
          ...
          if (!name) {
            ...
            const org = await orgResponse.json().catch(() => ({}));
            name = org.localizedName || org.vanityName || orgId;
            username = org.vanityName || username;
            picture =
  ...
  async fetchPageInformation(accessToken: string, params: { page: string }) {
    ...
    const data = await (
      await fetch(
        `https://api.linkedin.com/rest/organizations/${pageId}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
            'X-Restli-Protocol-Version': '2.0.0',
  ...
  override async authenticate(params: {
    ...
    body.append('redirect_uri', this.linkedinRedirectUri());
