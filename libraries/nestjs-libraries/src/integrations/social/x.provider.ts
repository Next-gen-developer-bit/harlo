...
export class XProvider extends SocialAbstract implements SocialProvider {
  ...
  override handleErrors(body: string):
    ...
    if (
      ...
      body.includes('The access token expired')
    ) {
      return {
        type: 'refresh-token',
  ...
  async refreshToken(refreshToken: string): Promise<AuthTokenDetails> {
    if (!refreshToken || !this.hasOAuth2()) {
      return {
        id: '',
        name: '',
        accessToken: '',
  ...
  async generateAuthUrl() {
    if (this.hasOAuth1()) {
      ...
      const { url, oauth_token, oauth_token_secret } =
        await client.generateAuthLink(this.redirectUri(), {
    ...
    if (!this.hasOAuth2()) {
      throw new Error(
        'X is not configured. Set X_CLIENT_ID and X_CLIENT_SECRET, or X_API_KEY and X_API_SECRET.'
      );
    }

    const { url, codeVerifier, state } =
      this.oauth2Client().generateOAuth2AuthLink(this.redirectUri(), {
        scope: this.scopes,
      });
  ...
  async authenticate(params: { code: string; codeVerifier: string }) {
    if (this.hasOAuth1()) {
      ...
      const profile = await this.loadProfile(client);
    ...
    const { client, accessToken, refreshToken, expiresIn, scope } =
      await this.oauth2Client().loginWithOAuth2({
        code: params.code,
  ...
  private async loadProfile(client: TwitterApi) {
    const {
  ...
  private isOAuth1AccessToken(accessToken: string) {
    return this.hasOAuth1() && accessToken.includes(':');
  }

  private splitOAuth1(accessToken: string) {
    const separator = accessToken.indexOf(':');
    return [
      accessToken.slice(0, separator),
      accessToken.slice(separator + 1),
    ] as const;
  }

  private async getClient(accessToken: string) {
    if (this.isOAuth1AccessToken(accessToken)) {
      const [token, secret] = this.splitOAuth1(accessToken);
      return new TwitterApi({
        appKey: process.env.X_API_KEY!,
        appSecret: process.env.X_API_SECRET!,
        accessToken: token,
  ...
  private authorization(method: string, url: string, accessToken: string) {
    if (this.isOAuth1AccessToken(accessToken)) {
      const [token, secret] = this.splitOAuth1(accessToken);
  ...
  private async finalizeArticle(
    ...
    const draftResponse = await this.fetch(draftUrl, {
      method: 'POST',
      headers: {
        Authorization: this.authorization('POST', draftUrl, accessToken),
    ...
    const publishResponse = await this.fetch(publishUrl, {
      method: 'POST',
      headers: {
        Authorization: this.authorization('POST', publishUrl, accessToken),
        'Content-Type': 'application/json',
      },
    });
  ...
  override async finalizePost(
    ...
    const tweetResponse = await this.fetch(tweetUrl, {
      method: 'POST',
      headers: {
        Authorization: this.authorization('POST', tweetUrl, accessToken),
        'Content-Type': 'application/json',
      },
