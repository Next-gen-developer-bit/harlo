...
export class StripeService {
  ...
  async syncCustomerEmailsAfterSwitch(
    ...
    await Promise.all(
      [...emailByCustomer].map(([customerId, email]) =>
        stripe.customers
          .update(customerId, {
            email: email.indexOf('@') > -1 ? email : `${email}@poscally.com`,
          })
          .catch(() => {})
      )
    );
  ...
  async createOrGetCustomer(organization: Organization) {
    ...
    const customer = await stripe.customers.create({
      email: users.users[0].user.email.indexOf('@') > -1 ? users.users[0].user.email : `${users.users[0].user.email}@poscally.com`,
  ...
  private async createEmbeddedCheckout(
    ...
    try {
      await stripe.customers.update(customer, {
        email: user.email.indexOf('@') > -1 ? user.email : `${user.email}@poscally.com`,
        ...(body.dub
          ? {
              metadata: {
